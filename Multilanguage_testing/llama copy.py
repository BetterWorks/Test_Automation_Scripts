...
supported_languages = languages.feedbacks
...
def get_rephrased_feedback(feedback):
    ...
    response = requests.post(feedback_api_url, json=payload, headers=headers)  # or however you're making the request
    response_data = response.json() 
    print(response_data) # Convert the response to a dictionary
    output_data = response_data['output'] if 'output' in response_data else response_data['message']
    suggestion_data = response_data['details'] if 'output' in response_data else response_data['message']
...
for lang, (feedback) in supported_languages.items():
    ...
    df.to_excel("multiLanguage/feedback_rephrasing_results_mistral.xlsx", index=False)

    # # Check for meaning consistency using cosine similarity
    # english_back_translated = "This is the back-translated English feedback."
